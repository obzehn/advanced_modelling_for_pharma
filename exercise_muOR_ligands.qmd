...
On the left, the solvated system after `gmx solvate`. On the right, the same system after removing ca. 1500 water molecules with the `water_removal.py` script. Protein, lipids and ligand are coloured as in @fig-muor-start, while water molecules in red (oxygen) and white (hydrogen).
:::
...
You can send the submission script `sbatch_me.sh` and let slurm handle the simulations by using the [sbatch](https://slurm.schedmd.com/sbatch.html) command. This should be run from Baobab's head-node, and not from the node that you received by running `salloc`. Thus, from the `GPCR_structure_X_excercise` directory where the submission script `sbatch_me.sh` is located, you can submit the job slurm with the following command
...
The panels of @fig-gpcr-pbc shows a few ways in which the trajectory can be post-processed. The videos were rendered with VMD. They are
