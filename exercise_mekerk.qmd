...
Final solvated dodecahedron box for the inactive MEK1-ERK2 active system. MEK1 and ERK2 are in a yellow and green cartoon representation, respectively. ATP is represented as sticks. Water is represented as a transparent soft blue surface. Ions are represented with their van der Waals radii and are in and violet (sodium and chlorine) and green (magnesium).
...
It is mandatory that `sbatch_me_MEKERK_inactive.sh`, `start.gro`, and `topol.top` are together in the same directory in which there are the energy minimization, NVT, NPT, and production directories, otherwise the script `sbatch_me_MEKERK_inactive.sh` won't be able to run the simulations for you.
...
Slurm reads this files and sends you in a queue while it waits to find some computer that is available and that has the requested hardware. When it finds it, it takes control of it for the amount of time specified in `--time 72:00:00`, that is, three days. You will need roughly double that amount of time to run the full production, but the sbatch script should take care of that by itself.

Continuing along the `sbatch_me_MEKERK_inactive.sh` script, as in the interactive case, the first thing to do once the node is allocated is to source GROMACS, which is the first thing done by the script with the lines following the `#SBATCH` node requests, i.e., with the following
...
You can send the submission script and let slurm handle the simulations by using the [sbatch](https://slurm.schedmd.com/sbatch.html) command. This should be run from Baobab's head-node, and not from the node that you received by running `salloc`. Thus, from the `MEKERK_inactive_exercise` directory where the submission script is located, you can submit the job slurm with the following command

```bash
sbatch sbatch_me_MEKERK_inactive.sh
```

If the command runs successfully, slurm returns the number that is assigned to your job, something like this

```bash
Submitted batch job 9152963
```
...
```bash
           ...
           9152963 private-g in_MEKER username  R       0:55      1 gpu039
```
...
Let's take a look at a few key entries in the mdp files.
...
Here you are asking GROMACS to use a [leapfrog integrator](https://en.wikipedia.org/wiki/Leapfrog_integration) by setting `integrator = md`. The integration is performed with a time step `dt` of 0.002 ps, that is, 2 fs, which is the standard for this type of simulations. Lastly, the integration is performed for 500000 steps, as set by `nsteps`, that is, `500000 x 0.002 ps = 1000 ps = 1 ns`. You can see that the number of steps is relatively short with respect to the production run, as reported in the `prod.mdp` parameters file.

The other most important section is the one where you set the thermostat and barostat settings. For the NVT runs, you have the thermostat that controls the temperature and the parameters look something like this (here taken from `nvt_1.mdp`)
...
; POSITION RESTRAINTS
;---------------------------------------------
define                       = -DPOSRES_ATP -DATP_k=1000 -DATP_P_k=1000 [...] -DPOSRES_mMg -DmMg_k=5000
```

The technical implementation and the specific meaning of these parameters is beyond the scope of this tutorial. The take at home message is that, due to the restraints, at the beginning both the ligands and the proteins (and the Mg ions) can't really move freely in space, but water can. In this way you can thermalize first the water molecules. Then, gradually, the restraints on the atoms of the system are decreased to let the whole system equilibrate. You can check the values of the parameters in the `POSITION RESTRAINTS` of the `nvt_X.mdp` and `npt_X.mdp` files. You will see that they gradually decrease up to disappearing in the last NPT equilibration.
...
Trajectories are fidgety things. The standard format of the GROMACS compressed trajectory, an `.xtc` file, contains the positions of the atoms as a function of time, collected as a sequence of frames. The frequency of the output is set by the keyword `nstxout-compressed` in the mdp files, in units of time steps. For the production run, this is 10000, that is, `10000 x 0.002 ps = 20 ps`. Considering that the run is long 500 ns, this means that you will collect 25000 frames. The content of the frames is defined by the keyword `compressed-x-grps`. Since this was not set in the mdp files, the default is the whole system, that is, the protein with water, ligands, and ions.
...
The panels of @fig-mekerk-pbc shows a few ways in which the trajectory can be post-processed. Please consider also that the dodecahedron box is represented internally as a triclinic like box, so the box shape might be different from what you expect. The panels are
...
- The trajectory corrected for PBCs broken molecules. This can be achieved with the flag `-pbc mol`. Notice how the protein is now whole, but GROMACS puts it back at the nearest interface with respect to its center, so the molecules can 'jump' across boundaries;
...
::: {#fig-mekerk-pbc layout-ncol=2}

::: {}
![](/images/image_mekerk/active_ori.gif){width="400" fig-align="center"}
:::

::: {}
![](/images/image_mekerk/active_pbc.gif){width="400" fig-align="center"}
:::

::: {}
![](/images/image_mekerk/active_cent.gif){width="400" fig-align="center"}
:::

::: {}
![](/images/image_mekerk/active_fit.gif){width="400" fig-align="center"}
:::

The same MEK1-ERK2 system trajectory re-wrapped in four different ways. The proteins are represented as cartoon ribbons, the ligands as sticks, and coordinating metal ions as van der Waals spheres. Water is removed for clarity. The first panel shows the original trajectory; the second shows molecules made whole across PBCs; the third centers the proteins in the box; the fourth aligns the protein complex to the last frame of the energy minimization. You can see that the ligand was bound for the whole duration of the production.
:::

For the exam, the main post-processing of the trajectory that you might need is the removal of the water molecules, as this will make the trajectory much smaller and easy to download and analyze. It is also worth to correct the PBCs and center the protein, which basically means obtaining what is shown in the third panel of @fig-mekerk-pbc. You can do this in a couple of passages. 

First, we need to cluster our system, since this is a dimer and sometimes the centering process splits the dimer between two PBC images. Just run 

```bash
gmx trjconv -f prod.xtc -s prod.tpr -pbc cluster -o prod_whole.xtc
```

and, when prompted by GROMACS, select `Protein` (usually `1`) as clustering group and `System` (usually `0`) as output. Then run

```bash
gmx trjconv -f prod_whole.xtc -s prod.tpr -pbc mol -center -o center_traj.xtc
```

Again select the group `Protein` for centering and `System` for output. The output will be called `center_traj.xtc` and will contain the whole trajectory where all the molecules are made whole across the PBCs (`-pbc mol`) and the protein is centered in the box (`-center`). You can then fit the protein to its starting position to better visualize the binding site (like panel 4 of @fig-mekerk-pbc) as in the following

```bash
gmx trjconv -f center_traj.xtc -s em.tpr -fit rot+trans -o center_traj_fit.xtc
```

and selecting the protein C-alphas for fit (usually group `3`) and the `System` for output. Now, `center_traj_fit.xtc` will contain the whole trajectory and after centering and fitting it on the starting configuration of the protein contained in `em.tpr`. This will probably still be a large file, so you can produce a more manageable file for visualization with VMD with the following two commands

```bash
gmx trjconv -f center_traj_fit.xtc -s prod.tpr -o nowater.xtc
gmx trjconv -f center_traj_fit.xtc -s prod.tpr -o nowater.pdb -dump 0
```

In both cases, select `non-Water` as the output group. These will produce two files, `nowater.pdb` and `nowater.xtc`, which are the first frame of the production run and the associated trajectory, respectively, but without the water. The files should be much smaller than the original `prod.xtc`. You can use these two files to visualize the trajectory in VMD. You can get rid of the intermediary files and remove them, that is, `rm center_traj.xtc center_traj_fit.xtc prod_whole.xtc`.
...
Two major points here. First, notice how the tpr passed to trjconv is `em.tpr` when we fit the system, the one compiled by the `sbatch_me.sh` script following the energy minimization. This is useful in cases where the refitting and re-centering is very complex due to the presence of more than one molecule which should stay consistent in terms of reference frame. Secondly, removing water might be a good idea for visualization purposes, but not for analysis ones. What about the interaction of water with the ligand/binding site? Are there any? Are they important? This is very relevant in systems like MEK1-ERK2, where the water molecules coordinating the magnesium ions are part of the physiological ligand-bound state. As such, use with care and keeping in mind what you want to do with the cleaned trajectory.
